...
public class Tree<T extends Comparable<T>> implements PriorityQueue<T> {
	 ...
	 *   
	 *   This method returns a Node instance, which for the inserted
	...
	public Node<T> insert(T thing) {
		...
		Node<T> ans = new Node<T>(thing, ++size);
		array[size] = ans;
		return ans;
	...
	/**
	 * Return the loc information from the Node stored at loc.  They
	 *   should agree. 
	 * @param loc
